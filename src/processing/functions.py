...
def filter_by_state(data: list, state: str = 'EXECUTED') -> list:
    ...
    # item.get('state') безопасен, даже если ключа нет
...
def sort_by_date(data: list, reverse: bool = True) -> list:
    ...
    # Функция-ключ для сортировки: превращает строку даты в объект datetime
    def get_date_key(item):
        date_str = item.get('date')
        if date_str:
            # fromisoformat работает со строками вида '2019-07-03T18:35:29.512364'
            return datetime.fromisoformat(date_str)
        # Если даты нет, считаем её самой старой (или можно вернуть None, но sorted может ругнуться)
        return datetime.min
...
if __name__ == "__main__":
    ...
    print("\n--- Тест sort_by_date ---")
    sorted_ops = sort_by_date(operations) # По умолчанию reverse=True (новые сверху)
